...
3. **重要**：确保数据库服务与应用服务在**同一个项目**中
4. 点击你的**应用服务**（不是数据库服务）
5. 转到 "Plugins"（插件）选项卡
6. 点击 "Add Plugin"（添加插件）
7. 选择你刚创建的数据库服务
8. 这将把数据库的环境变量注入到你的应用服务中
9. Railway会自动将数据库连接信息设置为 `DATABASE_URL` 环境变量
...
**重要更新**：我们已改进了数据库初始化流程，现在应用在启动时会自动执行数据库初始化，确保所有必需的表结构都已创建。
...
3. 确认DATABASE_URL中没有多余的空格或特殊字符
4. 查看应用日志中的详细错误信息
5. **最重要**：确认数据库服务已通过"Plugins"链接到应用服务

**Q: 为什么我看到 "ECONNREFUSED 127.0.0.1:5432" 错误？**
A: 这个错误表明应用没有找到数据库环境变量，尝试连接本地数据库。请确认：
1. 数据库服务和应用服务在同一个Railway项目中
2. 数据库服务已通过"Plugins"链接到应用服务
3. 应用服务的环境变量中包含DATABASE_URL
4. 重新部署应用服务以应用新的环境变量
